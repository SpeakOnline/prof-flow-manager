-- Migration: Require full interval coverage for advanced teacher search
--
-- Problem:
-- Matching only by intersecting hour numbers let shorter availability blocks
-- pass the search, even when they did not cover the full requested range.
--
-- Fix:
-- For multi-day and multi-range filters, require that each requested day has a
-- free schedule covering the full requested time range.
...
COMMENT ON FUNCTION public.search_teachers_advanced(
  ...
  UUID[],
  TEXT
) IS 'Busca avancada de professores com verificacao de cobertura completa para intervalos de disponibilidade.';
